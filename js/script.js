...
result.textContent = "正解！";
unlockPage(nextPage);
showPage(nextPage, 300); // 少しゆっくりスクロール
} else {
result.textContent = "違います。";
}
}
...
const tweetURL = "[https://twitter.com/intent/tweet?text=](https://twitter.com/intent/tweet?text=)" + encodeURIComponent(text) + "&url=" + encodeURIComponent(url);
